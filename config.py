...
class ExperimentConfig:
    model_name: str = "/root/jiarui/models/Qwen3-1.7B"
    model_name_short: str = "Qwen3-1.7B"

    # ── Quantization ──
    bits: int = 3
    group_size: int = 128
    desc_act: bool = False  # True breaks RoPE during quantization
    ...
    kd_batch_size: int = 2
    kd_grad_accum: int = 4
    kd_learning_rate: float = 2e-5
    kd_num_epochs: int = 1
    kd_max_steps: int = 200
    kd_temperature: float = 1.0
    kd_alpha_kl: float = 0.5
    kd_train_samples: int = 3000
    kd_warmup_ratio: float = 0.1
    kd_weight_decay: float = 0.01
    kd_lr_scheduler: str = "cosine"

    # ── Evaluation ──
    eval_max_new_tokens: int = 1024  # enable_thinking=False, answers are short
